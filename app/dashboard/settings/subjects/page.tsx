...
import { SubjectEditor, type EditorSubject } from '@/components/subjects/subject-editor'
import { canManageTenant, getSessionContext } from '@/lib/auth'
import { subjectSetVerdict } from '@/lib/subjects/moves'
import {
  isMissingSubjects, subjectCalibration, TABLE_SUBJECTS, type Subject,
...
export default async function SettingsSubjectsPage() {
  ...
  const rows = ((subjectRead.data ?? []) as Subject[]).map((s): EditorSubject => ({
    id: s.id,
    name: s.name,
  ...
  const verdict = subjectSetVerdict(rows.filter((s) => s.status === 'active').length)
  ...
  return (
    <SettingsFrame
      ...
      counts={available ? { subjects: String(rows.filter((s) => s.status === 'active').length) } : undefined}
    >
      <div className="flex flex-col gap-3">
        {!available ? (
          ...
          <>
            ...
            >
              <SubjectEditor subjects={named} canEdit={canEdit} />
            </SettingsCard>
            ...
            >
              {candidates.length === 0 ? (
                ...
                <SubjectEditor subjects={candidates} canEdit={canEdit} />
              )}
            </SettingsCard>
