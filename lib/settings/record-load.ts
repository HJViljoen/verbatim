...
export async function loadRailCounts(
  ...
  const counts: RailCounts = { record: `${updates} update${updates === 1 ? '' : 's'}` }
  ...
  if (!config.error && cfg) {
    ...
    if (terms > 0) counts.tracking = `${terms} term${terms === 1 ? '' : 's'}`
  }
  // A table that is not there is not a workspace with no subjects.
  if (!subjects.error && subjects.count != null && subjects.count > 0) counts.subjects = String(subjects.count)
  return counts
}
