...
async function storedEvidenceRefs(
  ...
  // `theme_id` on StoredFreeze is the month tables' third key column; this
  // table's key has four, so the composite travels in `key` and `theme_id`
  // carries the object id for the delete to aim at.
  return rows.map((r) => ({
    key: evidenceRefKey(r),
    month: monthStartOf(r.month),
    audience: r.audience,
    theme_id: `${r.object_kind}|${r.object_id}`,
    status: r.status,
    origin: r.origin,
...
export async function freezeEvidenceRefs(
  ...
  for (const s of merge.stale) {
    const [kind, ...rest] = (s.theme_id ?? '').split('|')
