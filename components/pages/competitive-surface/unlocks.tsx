...
// person who fixes it."
//
// AND ONE OF THE FOUR IS THE CLIENT'S TO FIX. CO4 reads a rival's own-post
// claims, which needs the rival's accounts configured — that is the digital
// director's, not engineering's, and saying "Verbatim engineering" against it
// would quietly take a job off the client's desk and put it on a queue.
//
          ...
          {data.unlocks.rows.map((row) =>
            email ? (
              <div key={row.section} style={{ padding: '5px 0', borderTop: `1px solid ${EMAIL.hairline}` }}>
                ...
                <div style={{ fontFamily: FONT.sans, fontSize: 11.5, color: EMAIL.muted, marginTop: 2 }}>— not tracked · {row.owner}</div>
              </div>
            ) : (
              <div key={row.section} className="flex min-w-0 flex-col gap-0.5 border-t border-border/70 pt-2">
                ...
                <p className="m-0 text-[11.5px] text-muted-foreground">— not tracked · {row.owner}</p>
              </div>
            ),
          )}
        </div>
