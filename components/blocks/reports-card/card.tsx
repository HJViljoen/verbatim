 ...
 *    `QuarterlyCard.ready` is `null` and stays `null`. The pill carries the
 *    GATE instead, which is the fact the artefact itself calls central.
...
export function QuarterlyCardTile({
  ...
  return (
    <Tile
      ...
      footerNote={card.gate}
    >
      <div className="flex items-center gap-2.5">
        <span
          className={`inline-block flex-none whitespace-nowrap rounded-full px-2 py-0.5 text-[12px] font-medium ${
            unlocked ? 'bg-inner text-secondary-foreground' : 'bg-warning/15 text-warning'
          }`}
        >
          {unlocked ? 'Six readings stand behind it' : MOVEMENT_WORDS.baseline_forming}
        </span>
          ...
          {card.series.map((s) => {
            ...
            return (
              <div key={s.label} className="flex min-w-0 flex-col gap-px">
                <div className="flex items-center gap-2 leading-[1.3]">
                  <span className="size-1.5 flex-none rounded-full" style={{ background: 'var(--cat)' }} aria-hidden />
                  <span className="min-w-0 flex-1 truncate text-[12.5px]">{s.label} · the category</span>
