...
| `purge-reports.ts` | Delete a workspace's reports: `--client <uuid> --scope drafts\|all`, dry by default, `--apply` deletes. `drafts` = everything never delivered (no `sent` send row, no share link ever opened) plus failed/orphan builds, artifacts + their Storage objects, export events, unopened share links, never-emailed `weekly_reports`, and definitions left with nothing; `all` adds the delivered history and breaks share URLs sitting in emails people already received. NEVER deletes a `reports` row that owns a `report_schedules` row (that FK cascades, and a lost default schedule is re-minted ACTIVE on the next accepted invite) and never writes `report_schedules` — an apply re-reads the table and compares it byte for byte. Refuses while a send is `claimed`/`ready` or a build is unfinished; warns about a send younger than 48h (one `report_missed` ops alert). The dry run prints every row that would go and every row that would stay, with the reason. Selection logic: `lib/reports/purge.ts` |
