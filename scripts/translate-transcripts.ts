...
import {
  MODEL_PRICING, SEALAND_CLIENT_ID, TRANSLATE_BATCH, TRANSLATE_CAP, TRANSLATE_MAX_CHARS, TRANSLATE_MODEL,
} from '../lib/config'
...
function parseArgs(argv: string[]): { clientId: string; limit: number | null; apply: boolean; prompt: boolean } {
...
async function report(clientId: string): Promise<Row[]> {
  ...
  for (const r of pending) {
    ...
    const chars = [...clipText(r.transcript ?? '', TRANSLATE_MAX_CHARS)].length
  ...
  console.log(`\nmodel ${TRANSLATE_MODEL} · $${price.inputPer1M}/$${price.outputPer1M} per 1M in/out · clipped at ${TRANSLATE_MAX_CHARS} chars`)
...
/** Input tokens from source characters, output tokens from the English
 *  rendering (Latin, so /4), which runs a little longer than the source. */
function costFor(chars: number, charsPerToken: number, price: { inputPer1M: number; outputPer1M: number }): number {
  ...
  const outTokens = (chars * 1.2) / CHARS_PER_TOKEN_LATIN
...
async function main() {
  ...
  if (apply && pending.length) {
    const take = pending.slice(0, limit ?? TRANSLATE_CAP)
